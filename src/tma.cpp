    ...
    switch (msg.type) {
        ...
        case DESCEND_REQUEST: {
            ...
            if(current_phase == CRUISING) {
                ...
                // Visual approach or not
                if(dest_airport->runways[landing_runway].active_stars.size() == 0) {
                    tma_ref->flights_on_service[std::string(msg.sender)] = DESCENDING_VFR;
                    snprintf(
                        message_text,
                        MESSAGE_SIZE,
                        "%s autorizado descida em voo VFR, esperada pista %s em %s\n",
                        msg.sender.c_str(),
                    ...
                    frequencies[tma_ref->radio_frequency].transmit(
                        ...
                        msg.sender.c_str(),
                        message_text,
                        (void *) &dest_airport->runways[landing_runway],
